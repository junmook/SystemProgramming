...
void run(unsigned char **memory, int progaddr, int endaddr){
2261     int x, y;
2262     int current_loc = progaddr;
2263     int content = 0;
2264     int temp = 0;
2265     Pcreg= progaddr;
2266
2267     while(1){
2268         x = current_loc/16;
2269         y = current_loc%16;
2270
2271         if(memory[x][y]==0xB4){
2272             x = (current_loc+1)/16;
2273             y = (current_loc+1)%16;
2274             if(memory[x][y]/16==0)
2275                 Areg = 0;
2276             else if(memory[x][y]/16==1)
2277                 Xreg = 0;
2278             else if(memory[x][y]/16==2)
2279                 Lreg = 0;
2280             else if(memory[x][y]/16==3)
2281                 Breg = 0;
2282             else if(memory[x][y]/16==4)
2283                 Sreg = 0;
2284             else if(memory[x][y]/16==5)
2285                 Treg = 0;
2286             else if(memory[x][y]/16==8)
2287                 Pcreg = 0;
2288             current_loc = current_loc+2;
2289         }
2290         //TIXR
2291         else if(memory[x][y]==0xB8){
2292             Xreg++;
2293             content = memory[(current_loc+1)/16][(current_loc+1)%16]/16;
2294             if(content==0){
2295                 if(Xreg==Areg)
2296                     temp = 0;
2297                 else if(Xreg>Areg)
2298                     temp = 1;
2299                 else
2300                     temp = -1;
2301             }
2302             else if(content==1){
2303                 if(Xreg==Xreg)
2304                     temp = 0;
2305                 else if(Xreg>Xreg)
2306                     temp = 1;
2307                 else
2308                     temp = -1;
2309             }
2310             else if(content==2){
2311                 if(Xreg==Lreg)

void run(unsigned char **memory, int progaddr, int endaddr){
	int x, y;
	int current_loc = progaddr;
	int content = 0;
	int temp = 0;
	Pcreg= progaddr;
	...
	while(1){
		...
		if(memory[x][y]==0xB4){
			...
			if(memory[x][y]/16==0)
				Areg = 0;
			else if(memory[x][y]/16==1)
				Xreg = 0;
			else if(memory[x][y]/16==2)
				Lreg = 0;
			else if(memory[x][y]/16==3)
				Breg = 0;
			else if(memory[x][y]/16==4)
				Sreg = 0;
			else if(memory[x][y]/16==5)
				Treg = 0;
			else if(memory[x][y]/16==8)
				Pcreg = 0;
			current_loc = current_loc+2;
		}
		//TIXR
		else if(memory[x][y]==0xB8){
			Xreg++;
			content = memory[(current_loc+1)/16][(current_loc+1)%16]/16;
			if(content==0){
				if(Xreg==Areg)
					temp = 0;
				else if(Xreg>Areg)
		...
		//COMPR
		else if(memory[x][y]==0xA0){
			temp = Compr(memory, current_loc);
			current_loc = current_loc+2;
			Pcreg = current_loc;
		}
		//LDA
		else if(memory[x][y]==0x00 &&(memory[(current_loc+1)/16][(current_loc+1)%16]!=0 ||memory[(current_loc+2)/16][(current_loc+2)%16]!=0)){
			if(decision_format(memory, current_loc)==4){
				current_loc = current_loc+4;
				Pcreg = current_loc;
			}
			else if(decision_format(memory, current_loc)==3){
				current_loc = current_loc+3;
				Pcreg = current_loc;
			}
		}
		//LDB
		else if(memory[x][y]==0x68){
			if(decision_format(memory, current_loc)==4){
				current_loc = current_loc+4;
				Pcreg = current_loc;
			}
		...
		//STX
		else if(memory[x][y]==0x10){
			pcchange(memory, &current_loc);
		}
		//STL
		else if(memory[x][y]==0x14){
			pcchange(memory, &current_loc);
		}
		//STCH
		else if(memory[x][y] == 0x54){
			pcchange(memory, &current_loc);
		}
		//J
		else if(memory[x][y] == 0x3c){
			Jump(memory, &current_loc);
		}
		//JSUB
		else if(memory[x][y]==0x48){
			jsub(memory, &current_loc);
		}
		else if(memory[x][y]==0x38){
			jlt(memory, &current_loc, temp);
		}
...
/*
void Jump(unsigned char **memory, int *current_loc){
	int target_addr, disp;
	int k = *current_loc;
	
	if(decision_format(memory, k)==4){
		target_addr = memory[(k+3)/16][(k+3)%16]+memory[(k+2)/16][(k+2)%16]*16*16+(memory[(k+1)/16][(k+1)%16]%16)*16*16*16*16;
	}
	//sic machine
	else if(decision_ni(memory, k)==0){
		target_addr = memory[(k+2)/16][(k+2)%16]+(memory[(k+1)/16][(k+1)%16]%128)*16*16;
		if(memory[(k+1)/16][(k+1)%16]/128==1){
			target_addr = target_addr+Xreg;
		}
	}
	//direct addressing
	else if(decision_xbp(memory, k)==0){
		disp = memory[(k+2)/16][(k+2)%16]+(memory[(k+1)/16][(k+1)%16]%16)*16*16;
		target_addr = disp;
	}
	//pc relative
	else if(decision_xbp(memory, k)==2){
		disp = memory[(k+2)/16][(k+2)%16]+(memory[(k+1)/16][(k+1)%16]%16)*16*16;
		if(disp>=0x800)
			disp = disp-0xFFF-0x1;
		target_addr = disp+k+3;
	}
	else if(decision_xbp(memory, k)==4){
		disp = memory[(k+2)/16][(k+2)%16]+(memory[(k+1)/16][(k+1)%16]%16)*16*16;
		if(disp>=0x800)
			disp = disp-0xFFF-0x1;
		target_addr = disp+Breg;
	}
	else if(decision_xbp(memory, k)==8){
		disp = memory[(k+2)/16][(k+2)%16]+(memory[(k+1)/16][(k+1)%16]%16)*16*16;
		target_addr = disp+Xreg;
	}
	else if(decision_xbp(memory, k)==10){
		disp = memory[(k+2)/16][(k+2)%16]+(memory[(k+1)/16][(k+1)%16]%16)*16*16;
		if(disp>=0x800)
			disp = disp-0xFFF-0x1;
		target_addr = disp+Xreg+k+3;
	}
	else if(decision_xbp(memory, k)==12){
		disp = memory[(k+2)/16][(k+2)%16]+(memory[(k+1)/16][(k+1)%16]%16)*16*16;
		if(disp>=0x800)
			disp = disp-0xFFF-0x1;
		target_addr = disp+Xreg+Breg;
	}

	if(decision_ni(memory, k)==0||decision_ni(memory, k)==3)
		Pcreg = memory[(target_addr+2)/16][(target_addr+2)%16]+memory[(target_addr+1)/16][(target_addr+1)%16]*16*16+memory[target_addr/16][target_addr%16]*16*16*16*16;
	else if(decision_ni(memory, k)==1)
		Pcreg = target_addr;
	else if(decision_ni(memory, k)==2){
		target_addr = memory[(target_addr+2)/16][(target_addr+2)%16]+memory[(target_addr+1)/16][(target_addr+1)%16]*16*16+memory[(target_addr)/16][target_addr%16]*16*16*16*16;
		Pcreg = target_addr;
	}
}
void jsub(unsigned char **memory, int *current_loc){
	int target_addr;
	int disp;
	int k = *current_loc;
	if(decision_format(memory, k)==4){
...
void pcchange(unsigned char **memory, int *current_loc){
	int k = *current_loc;
	int form = decision_format(memory, k);
	if(form==4){
		*current_loc = *current_loc+4;
		Pcreg = *current_loc;
	}
...
int Compr(unsigned char **memory, int current_loc){
	int reg1, reg2;
	int content1, content2;
	content1 = memory[(current_loc+1)/16][(current_loc+1)%16]/16;
	content2 = memory[(current_loc+1)/16][(current_loc+1)%16]%16;
